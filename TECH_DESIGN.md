...
| 作者 | Charlie-xylz |
| 创建日期 | 2026-09-20 |
| 依据文档 | `PRD.md` v1.3（Day 4 制定 / Day 9 路线对齐） |
| 本文档要回答的问题 | ① 数据从哪来、到哪去 ② 选哪条技术路线、为什么 ③ 各模块用什么实现 |
| 修订记录 | v1.0 首版（选定 PWA 路线）；v1.1 路线变更：改用 Flutter 做安卓 App，路线结论与全部技术选型重写 |
...
| 安卓厂商的后台省电策略可能延迟或拦截提醒 | F2 提醒不稳定，核心价值受损 | 引导用户把本 App 加入电池优化白名单；验收时在主力机上实测一次跨天提醒（PRD 9.2） |
| 需要真机调试，且工程路径不能含空格或中文 | 调试链路多一环，踩坑成本高 | 工程固定在 `D:\dev\today_todo`；用 USB 真机调试（本机未建模拟器） |
...
| --- | --- | --- | --- |
| **换技术路线：Web/PWA → Flutter 安卓 App**（2026-09-23 已发生） | 交付形态从网页变成 APK，运行环境从浏览器变成安卓系统，F2 的实现从 Push API 变成系统通知 | `PRD.md`（设备口径、6.1 加 iOS 条目、AC-19 改口径、原 AC-21 删除、9.2 风险表）；`TECH_DESIGN.md`（2.1～2.5、第三章全部、4.2）；`README.md`（几乎全改）；旧 Web 代码（`index.html`/`styles.css`/`app.js` 退役）；**新增** Flutter 工程 `D:\dev\today_todo` | 大改 + 新增 |
